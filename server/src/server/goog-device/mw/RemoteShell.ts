...
export class RemoteShell extends Mw {
    ...
    private timeoutString: any = null;
    private timeoutBuffer: any = null;
    private terminated = false;
    private closeCode = 1000;
    private closeReason = '';
    private logDevice?: string;
    ...
    protected onSocketMessage(event: WS.MessageEvent): void {
        if (this.initialized) {
            const device = this.logDevice || 'unknown';
            DeviceSocketLogService.log({ device, source: RemoteShell.TAG, data: event.data });
            if (!this.term) {
                return;
            }
        ...
        if (data && typeof data.udid === 'string') {
            this.logDevice = data.udid;
        }
        DeviceSocketLogService.log({
            device: this.logDevice || 'unknown',
            source: RemoteShell.TAG,
            data,
        });
